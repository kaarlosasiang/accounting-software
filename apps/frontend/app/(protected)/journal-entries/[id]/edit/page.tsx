...
import { JournalEntryStatus } from "@/lib/types/journal-entry";
...
export default function EditJournalEntryPage() {
  ...
  // Pre-fill form once entry data loads
  useEffect(() => {
    if (!entry) return;
    const prefilled: JournalLine[] = entry.lines.map((l) => ({
  ...
  const handleProceedWithWarning = () => {
    if (periodWarning.callback) periodWarning.callback();
    setPeriodWarning({
      show: false,
      periodName: "",
      periodStatus: "",
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      await updateMutation.mutateAsync({ id: entryId, data });
